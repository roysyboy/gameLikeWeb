...
window.addEventListener("load", function (event) {
  ...
  function render() {
    ...
    */
    ...
    // note.innerHTML = "(" + player.xOnCanv + ", " + player.yOnCanv + ")";
  ...
  */
  ...
  console.log('Start' + map.segX1 + ', ' + map.segY1);
  console.log(map.segX2 + ', ' + map.segY2);
  console.log(map.segX1 / CANV2BGSCALE + ', ' + map.segY1 / CANV2BGSCALE);
  console.log(map.segX2 / CANV2BGSCALE + ', ' + map.segY2 / CANV2BGSCALE);


  function playerMustMoveOnCanvas() {
    // execute some shite
    if (checkIfAtBorder()) {
      return true;
    } else if (checkIfAtBorder()) {
      return true;
    } else if (checkIfAtBorder()) {
      return true;
    } else if (checkIfAtBorder()) {
      ...
      // write down the coordinates of the player
    // note.innerHTML = "(" + player.xOnCanv + ", " + player.yOnCanv + ")";
    // document.body.appendChild(note);

    function playerMustMoveOnCanvas() {
    if (player.xOnMap < (map.borderLeft + player.speed)) {
      if (LEFT || (RIGHT && (player.xOnMap < (map.borderLeft - player.speed)))) {
        return true;
      }
    } else if (player.xOnMap > (map.borderLeft - player.speed)) {
      if (RIGHT || (LEFT && (player.xOnMap > (map.borderLeft + player.speed)))) {
        return true;
      }
    } else if (player.yOnMap < (map.borderUp + player.speed)) {
      if (UP || (DOWN && (player.yOnMap < (map.borderUp - player.speed)))) {
        return true;
      }
    } else if (player.yOnMap > (map.borderDown - player.speed)) {
      if (DOWN || (UP && (player.yOnMap < (map.borderDown + player.speed)))) {
        return true;
      }
    } else {
      return false;
